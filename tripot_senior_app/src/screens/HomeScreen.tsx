...
interface MenuItem {
  id: number;
  name: string;
  image: any; // require()의 반환 타입은 복잡하므로 any로 처리
  screen?: string; // screen은 없을 수도 있으므로 '?'를 붙여 optional로 만듭니다.
}
...
const HomeScreen = ({ navigation, connectionStatus }: HomeScreenProps) => {
  ...
  // handlePress 함수의 파라미터 타입을 명시합니다.
  const handlePress = (screenName?: string) => {
    if (screenName) {
      navigation.navigate(screenName);
    } else {
      console.log('이 기능은 아직 준비 중입니다.');
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.userName}>라기선님</Text>
        </View>
        <View style={styles.headerIcons}>
          <TouchableOpacity onPress={() => handlePress()} style={styles.iconButton}>
            <SvgXml xml={settingsIconSvg} width="100%" height="100%" />
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handlePress('Calendar')} style={styles.iconButton}>
      ...
      <View style={styles.statusContainer}>
        <Text style={styles.statusText}>서버 연결: {connectionStatus}</Text>
      </View>

      <View style={styles.menuContainer}>
        {menuItems.map((item) => (
          <TouchableOpacity key={item.id} style={styles.menuItem} onPress={() => handlePress(item.screen)}>
